  ...
  handler = function()
    ...
    local yandex_error_handler = function(code, api_context, request_type)
      ...
      if request.customerNumber ~= nil then
        api_context:ext("history.cache"):try_append(api_context, request.customerNumber, request_type .. " request: result code: " .. tostring(code))
      end
    ...
    local yandex_check = function(api_context, application, request)
      ...
      api_context:ext("history.cache"):try_append(api_context, request.customerNumber, " Check request: " .. tserialize(request))

      if request.action ~= "Check" then
        return fail("INCORRECT_INPUT", "[ya/check] incorrect action: " .. (request.action or "NIL"))
      end

      if request.orderIsPaid ~= 0 then
        return fail("INCORRECT_INPUT", "[ya/check] incorrect value of orderIsPaid")
      end
      ...
      local payment = api_context:ext("transactions.cache"):try_get(api_context,transaction_id)
      if next(payment) == nil then
        --TODO: need to search payment in request queue
        return fail("INTERNAL_ERROR", "[ya/check] transaction not found: ", transaction_id)
      end
      ...
      if request.orderSumAmount <= 0 or request.shopSumAmount <= 0 or request.shopSumAmount > request.orderSumAmount then
      ...
      api_context:ext("history.cache"):try_append(api_context, request.customerNumber, " Check request: result code: 0")
      log("[ya/check] check request accepted successfully for transaction : " .. transaction_id)
    ...
    local yandex_payment = function(api_context, application, request)
      ...
      api_context:ext("history.cache"):try_append(api_context, request.customerNumber, " Payment request: " .. tserialize(request))

      if request.action ~= "PaymentSuccess" then
        return fail("INCORRECT_INPUT", "[ya/paymentaviso] incorrect action: " .. (request.action or "NIL"))
      end

      if not request.orderIsPaid then
        return fail("INCORRECT_INPUT", "[ya/paymentaviso] incorrect value of orderIsPaid")
      end
      ...
      local payment = api_context:ext("transactions.cache"):try_get(api_context,transaction_id)
      if next(payment) == nil then
        return fail("INTERNAL_ERROR", "[ya/paymentaviso] transaction #" .. request.customerNumber .. " not found")
      end
      local src_t = tclone(payment)
      ...
      request.shopSumAmount = tonumber(request.shopSumAmount)
      if request.orderSumAmount <= 0 or request.shopSumAmount <= 0 or request.shopSumAmount > request.orderSumAmount then
        return fail("DATA_NOT_MATCH", "[ya/paymentaviso] incorrect received amounts")
      end
      ...
      payment.status = tonumber(payment.status) or PKB_TRANSACTION_STATUS.INVALID
      if payment.status ~= PKB_TRANSACTION_STATUS.CONFIRMED_BY_APP and payment.status ~= PKB_TRANSACTION_STATUS.CONFIRMED_BY_PAYSYSTEM  and payment.status ~= PKB_TRANSACTION_STATUS.CLOSED_BY_APP then
        local code = "WAIT_FOR_RESULT"
        if payment.status == PKB_TRANSACTION_STATUS.INVALID or payment.status == PKB_TRANSACTION_STATUS.REJECTED_BY_APP then
          code = "INCORRECT_STATUS"
        end
        return fail(code, "[ya/paymentaviso] incorrect status of payment: " .. payment.status)
      end

      if payment.payment_id ~= request.invoiceId then
        return fail("DATA_NOT_MATCH", "[ya/paymentaviso] Saved invoiceID not equals with received invoiceId")
      end
      if payment.status == PKB_TRANSACTION_STATUS.CONFIRMED_BY_PAYSYSTEM or payment.status == PKB_TRANSACTION_STATUS.CLOSED_BY_APP then
        api_context:ext("history.cache"):try_append(api_context, request.customerNumber, " Payment duplicate request: result code: 0")
      ...
      log("[ya/paymentaviso] payment request accepted successfully for transaction : " .. transaction_id)
      api_context:ext("history.cache"):try_append(api_context, request.customerNumber, " Payment request: result code: 0")
      api_context:ext("transactions.cache"):try_set(api_context, payment, src_t)
      return ya_build_response("OK", request)
    end
